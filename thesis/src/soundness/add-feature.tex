...
According to the semantics of assignment (Section~\vref{sec:maps}) and $\var{setFeatureAttributes}$ (Figure~\vref{fun:set-feature-attributes}), and given that $\lookup{\features'}{\var{featureID}} = \featurevar{F'}$, then 
\begin{align}
   ...
   \lookup{F'_c}{\interval{t_n}{t_m}} &= \emptyset && \text{the feature has subgroups} \tag{5} \label{add-feature5}
\end{align}
