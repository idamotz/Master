...
As mentioned in the previous section, there are aspects of our solution that can be improved upon. For an engineer to have complete freedom over her feature model evolution plan, one could create operations which let us extend and restrict the intervals in the interval-based feature model in all directions, not just for removal. Furthermore, the operations are not revertible. If we change the type of a feature from \mandatory{} to \optional{} at time $t_n$, and later wish to undo this change, we would apply the operation \textbf{changeFeatureVariationType} and change the type from \optional{} to \mandatory{}. This would result in a types map containing the mapping $\intervalmapping{t_n}{t_n}{\optional{}}$, i.e. an empty interval. An improvement for this is ignoring such mappings in the definition of the interval maps.

% Supporting batch operations
% Integrating the analysis in a tool such as DarwinSPL.
