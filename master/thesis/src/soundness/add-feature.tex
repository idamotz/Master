...
Since the rule checks that the feature does not already exist during the temporal scope, it is impossible that $\var{featureID} = \var{rootID}$. Thus the rule does not affect the root feature, and \wf{1} and \wf{2} hold for the updated temporal feature model.
...
The rule does not modify the parent group's variation type, so \wf{4} is preserved in the modified temporal feature model.
...
We conclude that the \rulefont{Add-Feature} rule preserves well-formedness for the temporal feature model, according to well-formedness rules \wf{1-9}. 
\\
\begin{lemma}
