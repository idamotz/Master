...
For any time point $t_n$ in the temporal scope of the rule, $t_n \notinn F_e$ due to the semantics of $\var{removeFeatureAt}$ (figure \vref{fun:remove-feature-at}), so \wf{3} and \wf{5} hold trivially. As the only change made to a group is by the function $\var{clampFeature}$ (figure \vref{fun:clamp-feature}), and since that function does not modify the types map $G_t$ of the group, \wf{4} holds given that it is true for the original model. 
