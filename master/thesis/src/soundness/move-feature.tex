...
Since the rule verifies that the feature has a parent group, the feature being moved is not the root. Thus \itbf{TFMWF1} and \itbf{TFMWF2} hold. The rule does not update the name, type or subgroups of the feature, so \itbf{TFMWF3} is true for the updated model. Nor does it modify the target group's type or parent feature, so \itbf{TFMWF4}, \itbf{TFMWF6}, and \itbf{TFMWF7} also hold.
...
\end{align*}
The feature is added to the target group by $\var{addChildFeature}$ during the interval $\interval{t_n}{t_{p_2}}$. Consequently \itbf{TFMWF5} holds for the updated model. 
\\
