...
\end{align*}
The feature is added to the target group by $\var{addChildFeature}$ during the interval $\interval{t_n}{t_{p_2}}$. Consequently \wf{5} holds for the updated model. 
...
After applying the \rulefont{Move-Feature} rule, if $\lookup{\features'}{\var{featureID}} = \featurevar{F'}$, then 
\[
   \lookup{F'_p}{\interval{t_n}{t_{p_2}}} = \set{\var{newParentID}}
\]
meaning that the feature is moved to the group with ID $\var{newParentID}$ during the temporal scope. An argument for this can be found in the previous paragraph.
\\
